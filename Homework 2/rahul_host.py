...
class GO:
    ...
    def detect_neighbor_ally(self, board, i, j):
        ...
        '''
        neighbors = self.detect_neighbor(self.board, i, j)  # Detect neighbors
        group_allies = []
    ...
    def find_liberty(self, board, i, j):
        ...
        '''
        ally_members = self.ally_dfs(self.board, i, j)
        for member in ally_members:
            neighbors = self.detect_neighbor(self.board, member[0], member[1])
            for piece in neighbors:
        ...
        for i in range(len(board)):
            for j in range(len(board)):
                ...
                if board[i][j] == piece_type:
                    ...
                    if not self.find_liberty(self.board, i, j):
    ...
    def remove_died_pieces(self, piece_type):
        ...
        died_pieces = self.find_died_pieces(self.board, piece_type)
    ...
    def place_chess(self, i, j, piece_type):
        ...
        valid_place = self.valid_place_check(self.board, i, j, piece_type)
        if not valid_place:
            return False
        self.previous_board = deepcopy(board)
    ...
    def valid_place_check(self, board, i, j, piece_type, test_check=False):
        ...
        test_go.board = deepcopy(board)
        test_board = test_go.board

        # Check if the place has liberty
        test_board[i][j] = piece_type
        test_go.update_board(test_board)
        if test_go.find_liberty(test_board, i, j):
            return True

        # If not, remove the died pieces of opponent and check again
        test_go.remove_died_pieces(3 - piece_type)
        if not test_go.find_liberty(test_board, i, j):
            if verbose:
