...
class MinMax:
    ...
    def update_heuristic_constants(self, tot_pieces):
    ...
    def all_heuristics(self, go, end):
        ...
        connects = self.connected_pieces(go, copy.deepcopy(pieces)) * self.connect_weight

        # Opp Liberty
        opp_lib = self.get_liberities(go, opp_pieces) *  self.opp_lib_weight
        opp_connects = self.connected_pieces(go, copy.deepcopy(opp_pieces)) *  self.opp_connect_weight
    ...
    def evalutaion(self, go):
        if(self.side == 1):
            ...
            multi = 1
        komi = -go.komi*multi if self.side == 1 else go.komi # Changing komi based on white or black
    ...
    def get_attack_list(self, empty_locs, go, side):
        ...
        for pos in empty_locs:
            ...
            go.board[pos[0]][pos[1]] = side
            opp_dead = go.find_died_pieces(3-side)
    ...
    def get_move(self, go):
        ...
        copy_attack = copy.deepcopy(my_attack_moves)
        copy_opp_attack = copy.deepcopy(opp_attack_moves)
        # Best move based on kills/saved
        best_move = {}
        for move in list(copy_attack.keys()):
            if move in list(copy_opp_attack.keys()):
                best_move[move] = copy_attack[move]/copy_opp_attack[move]
                del copy_attack[move]
                del copy_opp_attack[move]
        ...
        # Return best move that is offensive and defensive
        if(go.n_move <= 15 and len(best_move) != 0):
            return list(best_move.keys())[-1]
        elif(15< go.n_move <=18 and len(my_attack_moves) != 0):
            return list(my_attack_moves.keys())[-1]
        elif(19<go.n_move):
            if(len(my_attack_moves) != 0):
                return list(my_attack_moves.keys())[-1]
            elif(len(opp_attack_moves) != 0):
                return list(opp_attack_moves.keys())[-1]
        if(not empty_locs):
            return "PASS"
        else:
...
if __name__ == "__main__":
    ...
    if(go_board.compare_board(previous_board,board) and go_board.judge_winner() == piece_type):
        ...
        player = MinMax()
        player.update_heuristic_constants(num_pieces)
        player.set_side(piece_type)

        if(go_board.n_move <= 7 and piece_type == 1):
            action = player.fixed_moves_black(go_board)
        else:
